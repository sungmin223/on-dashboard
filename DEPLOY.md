...
- 프론트의 비밀번호 게이트는 클라이언트 측 SHA-256 해시 대조입니다. 통과 시 비밀번호 원문을 `sessionStorage` 에 보관합니다.
- **`/api/inventory`·`/api/chat` 는 서버측 인증으로 보호됩니다.** 요청 헤더 `x-access-token`(비밀번호 원문)의 SHA-256 이 `ACCESS_HASH` 와 일치해야 통과하며, 불일치 시 `401` 입니다.
  - 기본 `ACCESS_HASH` 는 프론트 게이트(`GATE_HASH`)와 동일하게 내장 → **무설정으로도 같은 비밀번호로 보호**됩니다.
  - 별도 API 비밀번호를 쓰려면 `.env` 의 `ACCESS_HASH` 에 그 해시를 지정하세요. 인증을 끄려면 빈 문자열(`ACCESS_HASH=`)로 두되, **공개 배포 시에는 비권장**입니다.
  - `/api/health` 는 의도적으로 공개(재고 건수·키 설정 여부만 노출).
- 추가 권장: 게이트 비밀번호는 충분히 길고 추측 어려운 값으로 설정하세요(해시는 클라이언트 번들에서 보이므로, 약한 비밀번호는 오프라인 추정에 취약).
- 더 강한 보호가 필요하면 호스트 레벨 접근 제어(IP 허용목록 / 인증 프록시)를 병행하세요.
